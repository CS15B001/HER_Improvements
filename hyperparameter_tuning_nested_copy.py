...
for env_i in range(len(env_name)):
	...
	for batch_i in range(len(batch_size)):
		...
		for replay_i in range(len(replay_k)):
			...
			os.system('nohup python3 -m baselines.her.experiment.train --num_cpu '+str(num_cpus)+' --env_name '+str(env_name[env_i])+' --logdir /home/gsarma8/HER_Improvements/'+time_of_run+' --n_epochs '+str(n_epochs[env_i]) \
