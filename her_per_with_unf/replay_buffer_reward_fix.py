    ...
    def store_episode(self, episode_batch):
        ...
        with self.lock:
            ...
            for ep in range(batch_size):
                # (s1, a, info_success, s2, g, t) - Right now storing t = 1000, random value
                # text = episode_batch.keys()
                # file_obj = open("srikanth_keyprint","w")
                # file_obj.write(str(text))
                # file_obj.close()
                for time_step in range(self.T):
                    transition = (episode_batch['o'][ep, time_step], episode_batch['u'][ep, time_step], episode_batch['info_success'][ep, time_step],
                                episode_batch['o_2'][ep, time_step], episode_batch['g'][ep, time_step], 1000)

                    # Store the transition with the actual goal
                    self.priority_queue.store(transition)

                    # Sampling alternate goals
                    rand = time_step + np.random.randint(self.T-time_step+1, replay_k)
                    # For each randomly sampled goal, add to priority_queue
                    for g in rand:
                        transition = (episode_batch['o'][ep, time_step], episode_batch['u'][ep, time_step], episode_batch['info_success'][ep, time_step],
                                    episode_batch['o_2'][ep, time_step], episode_batch['ag'][ep, rand], 1000)                      

                # Store in the priority_queue
                self.priority_queue.store(transition)
        # reward_params = {k: transitions[k] for k in ['g']}
