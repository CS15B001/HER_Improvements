...
class ReplayBuffer:
    def __init__(self, buffer_shapes, size_in_transitions, T, sample_transitions, conf):
    ...
    def store_episode(self, episode_batch):
        ...
        with self.lock:
            ...
            for t in range(batch_size):
                # (s1, a, r, s2, g, t) - Right now storing t = 1000, random value
                text = episode_batch.keys()
                file_obj = open("srikanth_keyprint","w")
                file_obj.write(str(text))
                file_obj.close()
                transition = (episode_batch['o'], episode_batch['u'], episode_batch['r'],
                                episode_batch['o_2'], episode_batch['g'], 1000)
